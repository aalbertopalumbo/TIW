...
@WebServlet("/ClonaConfigurazione")
public class ClonaConfigurazione extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (idStr == null) {
        		request.setAttribute("errore", "Impossibile clonare: ID della configurazione mancante.");
            request.getRequestDispatcher("/GoToLeMieConfigurazioni").forward(request, response);
            return;
        }

        try {
            new ConfigurazioneDAO(connection).clonaConfigurazione(Integer.parseInt(idStr));
        } catch (SQLException e) {
            e.printStackTrace();
            request.setAttribute("errore", "Errore durante la clonazione.");
            request.getRequestDispatcher("/GoToLeMieConfigurazioni").forward(request, response);
            return;
        }
